 ...
 */
class ReportUnit extends VReport
{
    public $task_type = Task::TYPE_UNIT;
    ...
    public function getCount($condition = null)
    {
        ...
        for($i = 0; $i < 365; $i++)
        {
            $time = $last + 86400 * $i;
            $day = date('Y-m-d', $time);
            $casePassed[$day] = $caseFailed[$day] = $caseCompleted[$day]
                    = $caseTotal[$day] = $ccRate[$day] = 0;
        }
    ...
    public function getLineHitRate()
    {
        $rate = Yii::t('VReport', 'Not Count');
        if($this->cc_line_total > 0)
        {
            $rate = TMath::getRate($this->cc_line_hit, $this->cc_line_total);
        }
        return $rate;
    }

    public function getCCResultHtml()
    {
        $commandRuns = CommandRun::model()->findAllByAttributes(array('task_run_id' => $this->task_run_id));
        $arr = array();
        foreach($commandRuns as $commandRun)
        {
            $rate = 'NA';
            if($commandRun->cc_line_total > 0)
            {
                $rate = TMath::getRate($commandRun->cc_line_hit,
                                $commandRun->cc_line_total);
            }
            if(!empty($commandRun->cc_result) && 'NA' != $commandRun->cc_result)
            {
                if('NA' == $rate) $rate = Yii::t('VReport', 'Not Toast Count');
                $arr[] = CHtml::link($rate, $commandRun->cc_result,
                                array('target' => '_blank'));
            }
            else
            {
