...
int CommandProcess::GetOutputSendToController(HANDLE h_Output)
{
    ...
    int total_timeout = INFINITE;
    if (m_agent_command->timeout != 0)
        total_timeout = m_agent_command->timeout *1000 * 60;
    ...
    while(1)
    {
        if(m_agent_command->timeout != 0)
        {
	        ...
	        if(time_elapsed >= total_timeout/1000)
	        {
		      // timeout
				result = 1;
				goto Cleanup;
		...
		if(m_agent_command->timeout != 0)
		{
			timeout = total_timeout - time_elapsed*1000;
			dwWait = WaitForSingleObjectEx(hEvent, timeout, TRUE);
		}
		else
		{
...
void HBThread::Run(void)
{
    ...
    while (!IsRequestStop())
    {
		...
		GetLoadInfo(&perf);
#endif
   // Log::Debug("userTime: %d", perf.userTime);
   // Log::Debug("systemTime: %d", perf.systemTime);
   // Log::Debug("idleTime: %d", perf.idleTime);
   // Log::Debug("totalMemory: %d", perf.totalMemory);
   // Log::Debug("freeMemory: %d", perf.freeMemory);
   // Log::Debug("totalDiskSpace: %d", perf.totalDiskSpace);
   // Log::Debug("freeDiskSpace: %d", perf.freeDiskSpace);
   // Log::Debug("diskRead: %d", perf.diskRead);
  //  Log::Debug("diskWrite: %d", perf.diskWrite);
  //  Log::Debug("inBytes: %d", perf.inBytes);
  //  Log::Debug("outBytes: %d", perf.outBytes);
   // Log::Debug("inPackets: %d", perf.inPackets);
  //  Log::Debug("outPackets: %d", perf.outPackets);
